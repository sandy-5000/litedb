...
void compact_page::adjust(std::shared_ptr<litedb::page::Page> page, std::vector<uint16_t> &positions) {
    ...
    for (int i = record_count; i-- > 0; ) {
        ...
        uint16_t* slot_entry = reinterpret_cast<uint16_t*>(
            page->data_ + constants::PAGE_HEADER_SIZE + i * sizeof(uint16_t)
        );
        *slot_entry = space_end;
    }
    page->header.free_space_offset = space_end;
    uint16_t free_space = space_end - (constants::PAGE_HEADER_SIZE + record_count * sizeof(uint16_t));
    std::memset(
        page->data_ + constants::PAGE_HEADER_SIZE + record_count * sizeof(uint16_t),
        0, free_space
    );
}
